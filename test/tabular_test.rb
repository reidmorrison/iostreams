    ...
    describe Tabular do
      describe '#parse_header' do
        it 'parses and sets the csv header' do
          tabular = IOStreams::Tabular::Tabular.new(format: :csv)
      ...
      describe '#cleanse_header!' do
        describe 'cleanses' do
          it 'a csv header' do
            tabular = IOStreams::Tabular::Tabular.new(columns: ['first field', 'Second', 'thirD'])
          ...
          it 'white listed snake cased alphanumeric columns' do
            tabular         = IOStreams::Tabular::Tabular.new(
        ...
        describe 'allowed_columns' do
          ...
          it 'passes' do
            tabular = IOStreams::Tabular::Tabular.new(columns: ['   first ', 'Second', 'thirD   '], allowed_columns: @allowed_columns)
            header  = tabular.cleanse_header!
          ...
          it 'nils columns not in the whitelist' do
            tabular = IOStreams::Tabular::Tabular.new(columns: ['   first ', 'Unknown Column', 'thirD   '], allowed_columns: @allowed_columns)
            header  = tabular.cleanse_header!
            assert_equal ['first', nil, 'third'], header
          end

          it 'raises exception for columns not in the whitelist' do
            tabular = IOStreams::Tabular::Tabular.new(columns: ['   first ', 'Unknown Column', 'thirD   '], allowed_columns: @allowed_columns, skip_unknown: false)
          ...
          it 'raises exception missing required columns' do
            ...
            tabular  = IOStreams::Tabular::Tabular.new(columns: ['   first ', 'Second', 'thirD   '], allowed_columns: @allowed_columns, required_columns: required)
            exc      = assert_raises IOStreams::Tabular::Errors::InvalidHeader do
              tabular.cleanse_header!
            end
          ...
          it 'raises exception when no columns left' do
            tabular = IOStreams::Tabular::Tabular.new(columns: ['one', 'two', 'three'], allowed_columns: @allowed_columns)
            exc     = assert_raises IOStreams::Tabular::Errors::InvalidHeader do
              tabular.cleanse_header!
            end
      ...
      describe '#parse' do
        before do
          @tabular = IOStreams::Tabular::Tabular.new(columns: ['first_field', 'second', 'third'])
        end
